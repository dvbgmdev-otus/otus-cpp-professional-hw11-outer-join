...
TEST_F(DatabaseTest, Insert_WhenIdIsNew_ReturnsInserted) {
    EXPECT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 1, "sweater"));
}
...
TEST_F(DatabaseTest, Insert_WhenIdAlreadyExistsInTable_ReturnsDuplicate) {
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 1, "sweater"));
    EXPECT_EQ(InsertResult::Duplicate,
              test_database.insert(Table::A, 1, "understand"));
}
...
TEST_F(DatabaseTest, Insert_WhenIdExistsInOtherTable_ReturnsInserted) {
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 3, "violation"));
    EXPECT_EQ(InsertResult::Inserted,
              test_database.insert(Table::B, 3, "proposal"));
}
...
TEST_F(DatabaseTest, Truncate_WhenTableHasRows_AllowsIdsToBeInsertedAgain) {
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 1, "sweater"));
    test_database.truncate(Table::A);
    EXPECT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 1, "understand"));
}
...
TEST_F(DatabaseTest, Truncate_WhenOtherTableHasRows_KeepsOtherTableUnchanged) {
    ...
    test_database.truncate(Table::A);
    EXPECT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 3, "quality"));
    EXPECT_EQ(InsertResult::Duplicate,
              test_database.insert(Table::B, 3, "example"));
}
...
TEST_F(DatabaseTest, Truncate_WhenTableBHasRows_AllowsIdsToBeInsertedAgain) {
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::B, 6, "flour"));
    test_database.truncate(Table::B);
    EXPECT_EQ(InsertResult::Inserted,
              test_database.insert(Table::B, 6, "wonder"));
}
...
TEST_F(DatabaseTest, Intersection_WhenTablesOverlap_ReturnsCommonRows) {
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 2, "frank"));
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 3, "violation"));
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 4, "quality"));
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::B, 3, "proposal"));
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::B, 4, "example"));
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::B, 6, "flour"));
    const std::vector<JoinedRow> rows = test_database.intersection();
...
TEST_F(DatabaseTest, Intersection_WhenRowsInsertedOutOfOrder_ReturnsRowsOrderedById) {
    for (const int id : { 5, 3, 4 }) {
        ASSERT_EQ(InsertResult::Inserted,
                  test_database.insert(Table::A, id, "A"));
        ASSERT_EQ(InsertResult::Inserted,
                  test_database.insert(Table::B, id, "B"));
    }
    const std::vector<JoinedRow> rows = test_database.intersection();
...
TEST_F(DatabaseTest, SymmetricDifference_WhenTablesOverlap_ReturnsExclusiveRows) {
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 0, "lean"));
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 3, "violation"));
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::B, 3, "proposal"));
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::B, 6, "flour"));
    const std::vector<JoinedRow> rows = test_database.symmetric_difference();
    ASSERT_EQ(2U, rows.size());
...
TEST_F(DatabaseTest, SymmetricDifference_WhenRowsInsertedOutOfOrder_ReturnsRowsOrderedById) {
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::B, 8, "selection"));
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 2, "frank"));
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::B, 7, "wonder"));
    ASSERT_EQ(InsertResult::Inserted,
              test_database.insert(Table::A, 1, "sweater"));
    const std::vector<JoinedRow> rows = test_database.symmetric_difference();
...
TEST_F(DatabaseTest, Insert_WhenTableIsUnknown_ThrowsInvalidArgument) {
    const auto unknown_table = static_cast<Table>(-1);
